...
> **Version 1.1 — figée le 19/09/2026 — lecture seule.**
> Référence métier du projet Clubify. Ne pas modifier : toute évolution passe par les fiches de
> `docs/features/` et par une entrée dans `docs/decisions/`. Les corrections de la version 1.1 sont
> listées dans l'historique des révisions (section 12).
...
| Exception | Lignes jaunes = inscrits datés de sept. 2024 à fév. 2025, donc de la saison précédente, réinscrits pour la saison 25-26 (confirmé par le club) | 1, 3 |
...
| P15 | Renouvellement : inscrits de la saison précédente recopiés et surlignés (img 1, confirmé) | Pas d'échéance visible, pas de relance | Campagne de réinscription par saison, place réservée jusqu'à une date | Relances échelonnées, libération automatique de la place | M |
...
| Reprise des inscrits de la saison précédente | img 1, 3 (confirmé) | Partiel (§4 renouvellement) | Campagne de réinscription et bascule de saison | M |
...
Point d'attention : votre document « brainstorming – functionnality » est conçu autour de la réservation à la séance (fenêtres de réservation, pénalités d'annulation, no-show, surbooking). Aucun document du club ne montre ce fonctionnement. Le club fonctionne par inscription d'un enfant dans un groupe récurrent pour une saison ; ce modèle doit être le cœur, la réservation à la séance devenant un cas secondaire mais réel, confirmé par le club : cours privés avec coach dédié (par exemple en boxe), carnets, essais, rattrapages, stages et camp d'été.
...
| TAR-04 | Remises automatiques | Fratrie : barème paramétrable par rang d'enfant (par défaut 5 % sur le 2e, 10 % sur le 3e, extensible au-delà) | GER | Appliquée automatiquement dès qu'une même famille a plusieurs enfants inscrits sur la saison ; assiette : prix des inscriptions de l'enfant, hors frais annuels ; rang : ordre d'inscription des enfants sur la saison ; barème, assiette et rang paramétrables par club ; le multi-activités relève du forfait, pas d'une remise ; cumul avec une remise manuelle soumis à plafond | P | S | MVP | FAM-01 |
...
| OFR-02 | Forfait combiné | N séances par semaine à répartir sur une liste d'activités, au prix réduit du forfait (colonne « 2 séances / semaine » des grilles actuelles), jamais par addition des prix unitaires ; calcul en cascade : prix explicite de la combinaison s'il existe, sinon prix « 1 séance » de l'activité la plus chère plus le supplément de chaque séance additionnelle ; un troisième mode, cumul diminué d'un pourcentage, reste disponible en configuration par club mais n'est pas retenu par défaut | GER | Le forfait crée une inscription par groupe choisi mais une seule ligne de prix sur la facture ; l'écart avec le cumul des prix unitaires s'affiche comme économie, il ne se saisit pas comme remise | D | M | MVP | OFR-01, INS-02 |
| OFR-03 | Règles de combinaison | Nombre maximum de séances par activité (configurable) ; la même activité deux fois dans la semaine, dans deux groupes différents, est un cas standard du forfait (colonne « 2 séances / semaine »), sur place disponible dans chaque groupe ; incompatibilités horaires ; aucune exclusion par nature, l'indisponibilité venant des groupes complets | GER | Contrôlées à l'inscription ; au MVP, un groupe complet n'accepte plus d'inscription et une activité dont tous les groupes éligibles sont complets est affichée comme indisponible, non masquée ; la liste d'attente arrive en V2 (RES-02) ; dérogation du gérant avec motif pour l'âge ou le niveau (GRP-04) | A | M | MVP | OFR-02 |
...
| UC13 | Changement de groupe ou de niveau | Évaluation → proposition → place disponible → transfert daté → parent informé | Si le groupe cible est plein : transfert refusé au MVP, liste d'attente en V2 | MVP / V2 |
...
- [x] Que signifient les lignes jaunes de la liste d'inscription : inscrits de la saison précédente à renouveler, ou paiement soldé ? Confirmé : ce sont les familles réinscrites pour la saison suivante. Point clos.
...
- [x] Remises confirmées : fratrie à 5 % sur le deuxième enfant et 10 % sur le troisième, barème à rendre paramétrable ; le multi-activités passe par le prix de forfait (OFR-02) et non par une remise. Tranché le 19/09/2026 : le pourcentage porte sur le prix des inscriptions de l'enfant, hors frais annuels ; le rang suit l'ordre d'inscription des enfants sur la saison. Valeurs par défaut, paramétrables par club. Point clos.
...
- [x] Le même enfant peut-il venir 2 fois par semaine dans deux groupes différents de la même activité ? Révisé le 19/09/2026 : oui, c'est un cas standard du forfait 2 séances par semaine, l'enfant choisissant deux fois la même activité dans deux groupes différents, à condition qu'une place soit libre dans chaque groupe. Aucune dérogation requise. Point clos.
- [x] Activités réellement ouvertes cette saison : Ninja Warrior, danses, Genius Club, Kick-Boxing et Karaté ont-ils des groupes ? Confirmé : danse classique, Zumba Kids et Afro Dance (trois activités distinctes), karaté, boxe et natation, plus celles des classeurs Excel, judo, gymnastique et kick-boxing. Ninja Warrior et Genius Club ne sont pas retenus, sauf indication contraire. Point clos.
...
| --- | --- | --- |
| 1.0 | 19/09/2026 | Version initiale, figée. |
| 1.1 | 19/09/2026 | Mise en cohérence avec les décisions `docs/decisions/0001` à `0022`. Lignes jaunes : familles réinscrites (1.4, P15, section 3, section 11). Groupe complet : refus d'inscription au MVP, liste d'attente en V2 (P4, OFR-03, UC13, section 11). Forfait : cascade de calcul retenue, mode « cumul moins pourcentage » optionnel (OFR-02, section 11). Même activité deux fois par semaine : cas standard du forfait, plus une dérogation (OFR-03, section 11). Remise fratrie : assiette hors frais annuels, rang par ordre d'inscription (TAR-04, section 11). Catalogue : trois activités de danse distinctes (section 11). Camp d'été ajouté aux cas de réservation à la séance (section 3). |
