...
- Permissions fines (SEC-02) : `@PreAuthorize` sur les services (pas seulement sur les contrôleurs) pour finances, remises, dérogations, santé. Rôles : gérant, administratif, coach, comptable, parent.
...
- ArchUnit : les règles de dépendance de l'arborescence, « aucun `@RestController` ne dépend d'un `Repository` », « aucune logique dans `controller` ».
